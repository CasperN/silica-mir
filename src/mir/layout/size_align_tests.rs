...
fn scalar_sizes() {
    ...
    assert_eq!(
        size_of(&Type::synthesized(TypeKind::Int(IntTy::I64)), &env),
        8
    );
...
fn scalar_alignments() {
    let env = env_of("fn f() { entry: return }");
    assert_eq!(
        align_of(&Type::synthesized(TypeKind::Int(IntTy::I64)), &env),
        8
    );
...
fn all_ref_kinds_and_fn_are_pointer_sized() {
    let env = env_of("fn f() { entry: return }");
    let inner = Box::new(Type::synthesized(TypeKind::Int(IntTy::I64)));
    for kind in [
        RefKind::Shared,
        RefKind::Mut,
        RefKind::Out,
        ...
        let ty = Type::synthesized(TypeKind::Ref(kind, None, inner.clone()));
...
fn empty_struct_is_zero_sized() {
    ...
    let ty = Type::synthesized(TypeKind::Custom("S".to_string(), Vec::new(), Vec::new()));
...
fn homogeneous_struct_sums_field_sizes() {
    ...
    let ty = Type::synthesized(TypeKind::Custom("P".to_string(), Vec::new(), Vec::new()));
...
fn nested_struct_inherits_alignment() {
    ...
    let ty = Type::synthesized(TypeKind::Custom(
        "Outer".to_string(),
        Vec::new(),
        Vec::new(),
    ));
...
fn enum_with_unit_only_variants_is_discriminant_only() {
    ...
    let ty = Type::synthesized(TypeKind::Custom("E".to_string(), Vec::new(), Vec::new()));

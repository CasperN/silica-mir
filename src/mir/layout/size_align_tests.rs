...
fn empty_struct_is_zero_sized() {
    ...
    let ty = Type::synthesized(TypeKind::Custom(Instance::bare("S")));
...
fn homogeneous_struct_sums_field_sizes() {
    ...
    let ty = Type::synthesized(TypeKind::Custom(Instance::bare("P")));
...
fn nested_struct_inherits_alignment() {
    ...
    let ty = Type::synthesized(TypeKind::Custom(Instance::bare("Outer")));
